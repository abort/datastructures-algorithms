...
object Dijkstra {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(computeShortestPathWeight(line.head, line(1), edges, weights) match {
      case Finite(v) => v
      case Infinite => -1
    })
  }
  ...
  private def computeShortestPathWeight(u : Int, v: Int, edges: Edges, weights: Weights) : Distance = {
    val vertices = edges.size
    var distances = IndexedSeq.fill[Distance](vertices)(Infinite)
    //var edgeTo =
    var queue = new IndexMinPQ[Distance](vertices)
    distances = distances.updated(u, Finite(0))

    queue.insert(u, distances(u))
    while (!queue.isEmpty) {
      val minVertex = queue.delMin()

      edges(minVertex).zipWithIndex.foreach { case (w, index) =>
        val computedDistance = distances(minVertex) + weights(minVertex)(index)
        if (distances(w) > computedDistance) {
          distances = distances.updated(w, computedDistance)
          // no need for prev?
          if (queue.contains(w)) queue.changeKey(w, computedDistance)
          else queue.insert(w, computedDistance)
        }
      }
    }
