...
object Clustering {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(computeMaximumClusterDistance(allPoints.map(p => (p, allPoints)).toMap, k))
  }


  private def computeMaximumClusterDistance(edges: Edges, k : Int) : Double = {
    ...
    for ((p, q) <- sortedPairs) {
      ...
      if (clusters != k - 1) {
        ...
        if (pointsSets.find(pIndex) != pointsSets.find(qIndex)) {
          // if (clusters == k) return Math.abs(p - q)
